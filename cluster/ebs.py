...
Before running script export AWS_REGION & AWS_PROFILE file as env:
    ...
    parser.add_argument(
        ...
        "--delete",
        action="store_true",
        help="if flag set, deletes the volume for specific cluster",
    )
    parser.parse_args()
    ...
    def delete_volumes(cluster, sort, delete):
        if delete:
            if cluster:
                ...
                logger.warning(
                    "Please pass name of cluster for which ebs volumes need to be deleted!"
                )
                if "y" in input(
                    "Do you want to get ebs volumes details for all clustes? y|n: "
                ):
