...
class Output:
    ...
    def summary(data_len, object_type):
        print(
            Output.PURPLE
            + "\nTotal {} count: ".format(object_type)
            + Output.RESET
            + str(data_len)
        )

    # prints table from lists of lists: data
    def print_table(data, headers, border):
        try:
            if len(data) != 0:
                headers = headers[:6]
    ...
    def print_tree(data, headers):
        h = sorted(
            headers[1:], key=len
        )  # sorting to find longest element in headers for :
        for d in data:
            heading = headers[0] + ": "
                    ...
                    if not "\n" in d[i + 1]:
                        ...
                        for x in d[i + 1].split("\n"):
                            ...
                            if "[" in x:
                                ...
                                for j in x:
                                    y_padding = len(
                                        i_padding + Output.MARKER + previous
                                    )
                                    if y_padding > 50:
                                        y_padding = 50
