class GetElb:
    def get_elb(session, cluster):
        ...
        for elb in elb_list["LoadBalancerDescriptions"]:
            ...
            elb_attributes = elb_client.describe_load_balancer_attributes(
                LoadBalancerName=elb_name
            )
            try:
                if type(
                    elb_attributes["LoadBalancerAttributes"]["CrossZoneLoadBalancing"][
                        "Enabled"
                    ]
                ) == type(True):
                    elb_cross_zone = "True"
                ...
                elb_cross_zone = ""
            try:
                elb_access_logs = elb_attributes["LoadBalancerAttributes"]["AccessLog"][
                    "Enabled"
                ]
            except KeyError:
                elb_access_logs = ""
            try:
                elb_bucket_name = elb_attributes["LoadBalancerAttributes"]["AccessLog"][
                    "S3BucketName"
                ]
            except KeyError:
                elb_bucket_name = ""
            try:
                elb_bucket_prefix = elb_attributes["LoadBalancerAttributes"][
                    "AccessLog"
                ]["S3BucketPrefix"]
            except KeyError:
                elb_bucket_prefix = ""
            ...
            elb_details = elb_client.describe_tags(
                LoadBalancerNames=[
                    elb_name,
                ]
            )
