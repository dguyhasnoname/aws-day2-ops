...
Before running script export AWS_REGION & AWS_PROFILE file as env:
    ...
    parser.add_argument(
        ...
        "--cluster",
        action="store_true",
        help="cluster name for which details has to be fetched",
    )
    parser.add_argument(
        "-u",
        "--update",
        action="store_true",
        help="asg group type. Valid inputs etcd|master|worker|all",
    )
    parser.parse_args()
    ...
    def update_asg_details(cluster, session, output, sort, update):
        if update:
            if cluster:
                ...
                logger.warning(
                    "Please pass name of cluster for which asg needs to be updated!"
                )
                logger.info("Pulling asg details for all clusters.")
                if "y" in input(
                    "Do you want to get asg details for all clustes? y|n: "
                ):
                    ASG.get_asg_details(cluster, session, output, sort)
